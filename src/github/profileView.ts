import {API_ERROR, API_LIMIT_REACHED, findLicense, FOUND_IGNORED_REPO} from "./licenseFinder";
...
async function main() {
    ...
    for (let pin of Array.from(pins)) {
        ...
        const alarmReport = await getAlarm(license);
        addLicenseInfoToPin(pin, alarmReport);
    }
}
...
function addLicenseInfoToPin(pin: Element, alarmReport: AlarmReport) {
    if (alarmReport.licenseKey === API_LIMIT_REACHED || alarmReport.licenseKey === API_ERROR) {
        console.log(`[licenseplate] ${alarmReport.licenseKey} - not placing any badges.`);
        return
    }
